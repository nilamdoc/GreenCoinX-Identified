...
{
    QString ret = QString("greencoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XGC unit input in lnReqAmount, we generate the URI with XGC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::XGC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
