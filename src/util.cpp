...
#else
    const char* pszModule = "greencoin";
#endif
    if (pex)
        return strprintf(
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No greencoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override greencoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "greencoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Greencoin will not work properly.");
